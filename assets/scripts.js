...
var apiKeyYouTube = "AIzaSyDdPPp3lg3nt_5w6B1KAOkosMq-ZfV9OSc";
//max search history
...
function updateYouTubeDom(results) {
    ...
    for (let i = 0; i < maxLength; i++) {
        const ele = results[i];
        html =
            "<div class='column is-one-fifth has-text-centered'><a href='https://www.youtube.com/watch?v=" +
            ele.videoId +
            "' target='_blank'><div class='card large'><div class='card-image'><figure class='image'><img src='" +
            ele.image.url +
            "' alt='" +
            ele.title +
            "'></figure></div><div class='card-content'><div class='media'><div class='media-content'><p class='card-title'>" +
            ele.title +
            "</p><p class='card-info'>" +
            ele.created +
        ...
        .then((data) => {
            for (let i = 0; i < data.items.length; i++) {
                ...
                var resultObject = {
                    ...
                    created: moment(ele.snippet.publishTime).format(
                        "M-DD-YYYY"
                    ),
...
function getTrendingSingles(rankThreshold, year, genre, apiKey) {
    ...
    var constructedUrl =
        "https://api.discogs.com/database/search?genre=" +
        genre +
        "&year=" +
        year +
        "&format=single&token=" +
...
function saveRelevantGenres(resultAlbums) {
    ...
    localStorage.setItem(
        "savedGenres",
        JSON.stringify(Array.from(savedGenres))
    );
}
...
function updateDomFromDicogs(resultAlbums, maxResults, elementUpdated) {
    ...
    for (let i = 0; i < maxResults; i++) {
        ...
        html =
            "<div class='column is-one-fifth has-text-centered'><div class='card large modal-click'><div class='card-image'><figure class='image'><img class='content-image' src='" +
            ele.image +
            "' alt='" +
            ele.title +
            "'></figure></div><div class='card-content'><div class='media'><div class='media-content'><p class='card-title'>" +
